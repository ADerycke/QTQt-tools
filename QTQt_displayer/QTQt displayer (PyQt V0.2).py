...
def extract_FT_Length(data):
    ...
    #handle modification from QTQt version to find the position of LFT data
    ...
    for index, row in  FT_Length_tab[FT_Length_tab.type.str.contains('Max_Like')].iterrows():
        ...
        #upscaling the table from 10 to 100 rows to fit all data in the same matrix (pred)  
        all_row = numpy.empty((20,10,))
        all_row[:,:] = numpy.nan
        all_row[:,0] = obs_LFT.bar
