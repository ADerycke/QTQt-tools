...
# load color liste for sample
global nom_color
tab_color = {}
n=1
for item, value in matplotlib.colors.TABLEAU_COLORS.items():
    nom = "files " + str(n)
    tab_color[nom] = mcolors.hex2color(value)
    n = n + 1
        ...
        for i in range(data_He.shape[0]):
            ...
            for j in range(len(x)):  #obligation d'itération pour le cas où des cristaux sont différents
                ...
                plot_age.scatter(x[j] , y[j], s=30,
                                 ...
                                 edgecolors = "black",
                                 alpha=0.50)
                plot_age.errorbar(x[i] , y[i], xerr=dx[i], yerr=dy[i], fmt=crystal_type, markersize=0, ecolor=color_list[nom])
